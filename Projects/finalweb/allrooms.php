    ...
    <?php 
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                echo "<button type='button' class='btn btn-primary me-2' data-bs-toggle='modal' data-bs-target='#exampleModal".$row["rm_id"]."'>
                แก้ไข </button>";
                echo "<a href='deleteRm.php?rm_id=".$row["rm_id"]."' class='btn btn-danger'>ลบ</a>";
                ?>
                <div class="modal fade" id="exampleModal<?php echo $row["rm_id"]; ?>" tabindex="-1" aria-labelledby="exampleModalLabel<?php echo $row["rm_id"]; ?>" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel<?php echo $row["rm_id"]; ?>">แก้ไขห้องพัก</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                            <form action="editRm.php" method="POST">
                              <div class="mb-3">
                                  <label class="form-label">หมายเลขห้อง</label>
                                  <input type="text" class="form-control" name="rm_id" value="<?php echo $row["rm_id"]; ?>">
                              </div>
                              <label class="form-label">ประเภทห้อง</label>
                              <input type="text" class="form-control" value="<?php echo $row["tr_name"]; ?>" readonly>
                              <div class="mb-3">
