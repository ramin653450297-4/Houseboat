    ...
    <div class="card text-center">
        <div class="card-header">
            <ul class="nav nav-tabs card-header-tabs">
                <li class="nav-item">
                    <a class="nav-link active" aria-current="true" href="cusbooking.php">จองห้อง</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="cusbookingmenu.php">จองอาหาร</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            ...
            if(isset($_SESSION['cus_user'])) {
                ...
                $sql = "SELECT * FROM detailbooking WHERE cus_name = '$cus_name'";
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo "<h5 class='card-title'>ข้อมูลการจอง</h5><br>";
                        echo "<p class='card-text'>หมายเลขห้อง " . $row["rm_id"] . "</p>";
                        echo "<p class='card-text'>ชื่อ " . $row["cus_name"] . "</p>";
                        echo "<p class='card-text'>วันที่เช็คอิน " . thaiDate($row["check_in"]) . "</p>";
                        echo "<p class='card-text'>วันที่เช็คเอ้าท์ " . thaiDate($row["check_out"]) . "</p>";
                        echo "<p class='card-text'>สถานะการชําระเงิน " . $row["status"] . "</p>";
                        echo "</div>"; // Closing card-body div
                    }
                } else {
                    echo "<h1>ไม่มีข้อมูลการจอง</h1>";
                }
